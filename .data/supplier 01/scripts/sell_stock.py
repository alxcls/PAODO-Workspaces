"""
sell_stock.py — Process a sale order: deduct stock, log the transaction.

Usage:
    python scripts/sell_stock.py <ingredient> <quantity> [<buyer>]

Example:
    python scripts/sell_stock.py mozzarella 500 "pizza shop 01"
"""
...
ORDERS_LOG_FILE = BASE / "data" / "orders_log.json"
...
def main():
    ...
    log = json.loads(ORDERS_LOG_FILE.read_text())
    order = {
    ...
    log["orders"].append(order)
    ORDERS_LOG_FILE.write_text(json.dumps(log, indent=2))
