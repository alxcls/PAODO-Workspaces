...
export default function ModelBlock({ wsId }: { wsId: string }) {
  ...
  const [saved, setSaved] = useState<{ provider: string; model: string; effort: string }>({
    provider: DEFAULT.provider, model: DEFAULT.model, effort: DEFAULT.reasoningEffort,
  });
  ...
  useEffect(() => {
    fetch(`/api/workspaces/${wsId}`)
      ...
      .then((d: { llmProvider?: string; llmModel?: string; reasoningEffort?: string }) => {
        ...
        setProvider(p); setModel(m); setEffort(e);
  ...
  const W = { provider: 128, model: 168, effort: 100 };

  // A committed value shown when not editing: a greyed, caret-less read-only field so it clearly
  // reads as a set value rather than an interactive dropdown.
  const Locked = ({ value, width }: { value: string; width: number }) => (
    <div
      style={{ width }}
    ...
    <div className="flex flex-col gap-3 mt-4 border border-border rounded-card p-[14px_16px] bg-bg-tint">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {editing ? (
          ...
          <>
            <Locked value={provider} width={W.provider} />
            <Locked value={model} width={W.model} />
            {efforts.length > 0 && <Locked value={effort} width={W.effort} />}
