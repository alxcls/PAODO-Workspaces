...
export default function HtmlLivePreview({ workspaceId, base, draft, filePath }: Props) {
  ...
  const htmlForPreview = useMemo(() => {
    ...
    const html = /<head(\s[^>]*)?>/.test(draft)
      ...
      : baseTag + draft;
    return html;
  }, [draft, filePath, base, previewToken]);
  ...
  return (
    <iframe className="html-preview" srcDoc={htmlForPreview}
