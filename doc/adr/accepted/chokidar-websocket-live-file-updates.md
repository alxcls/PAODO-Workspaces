...
- `files_changed`: silently re-fetches the open file's content, unless the user has unsaved edits (`isDirtyRef`). Bumps a `previewKey` counter for HTML/JSON to force iframe/JSONCrack remount.
