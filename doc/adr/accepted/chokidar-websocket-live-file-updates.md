...
- Batches `add`/`change` events with a 150 ms debounce before broadcasting `{ type: "files_changed", paths }`.
...
On the client, `FileViewer` opens its own WebSocket connection and:
- On `files_changed`: silently re-fetches the open file's content, unless the user has unsaved edits (`isDirtyRef`). Bumps a `previewKey` counter for HTML/JSON to force iframe/JSONCrack remount.
- On `files_deleted`: closes the viewer if the deleted path matches the open file.
