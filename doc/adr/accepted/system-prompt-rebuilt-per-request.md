...
The system prompt is assembled from disk (`lib/agent/systemPrompt.ts`) on every call to the chat route (`app/api/workspaces/[id]/chat/route.ts`), not cached. `AGENTS.md` is read fresh each time via `fs.readFile`.
...
- The system prompt is not part of the stored message history, so it never inflates the conversation array.
