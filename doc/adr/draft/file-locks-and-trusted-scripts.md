...
- ~~Route file tools through Docker: subprocess overhead on every op, no locking benefit.~~ **Implemented**: file tools now go through `docker exec` for OS-level isolation. The subprocess overhead (~20–50ms per call on a warm container) is acceptable; the security benefit (unified OS boundary, global lock enforced at OS level for all operations) outweighs it.
