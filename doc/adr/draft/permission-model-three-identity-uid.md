...
**Eye restriction — data and config files only.** Eye is blocked for executable scripts (extensions in `EXECUTABLE_EXTENSIONS`, `lib/utils/fileType.ts`, derived from the `RUNTIME_PREFIX` dispatch table). The Linux kernel cannot execute an interpreted script without read permission: it must read the shebang and the interpreter reads the body. Hiding a script makes it silently unexecutable. Enforced at two layers: (1) `EyeBadge` is not rendered when `isExecutable(node.name)` or `node.privileged` is true; (2) `PATCH /hidden` returns 400 for executable extensions. Scripts requiring content protection should use Lock (write-protect) combined with Key (privd execution).
