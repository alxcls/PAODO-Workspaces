...
|----------|-------|--------|
| Command output has no enforced size cap — a verbose build or a large file dump could flood the agent's context. Should we truncate at a threshold and surface a warning? | @alxcls | Open |
| The task list is in-memory only and resets on server restart. Should it be persisted so a resumed session recovers its checklist? | @alxcls | Open |
| The agent cannot tell when a fetched URL redirected — should we surface the final landing URL so it can reason about where it ended up? | @alxcls | Open |
| Should the agent be able to spawn a one-shot reasoning call to a clean LLM instance (no workspace context, no tools) for a second opinion? | @alxcls | Open |
| As the capability list grows, how do we prevent the agent from making worse tool choices? Research suggests quality degrades past ~15 tools. | @alxcls | Open |
| The agent can bypass a file lock by writing a script that modifies the locked file and running it as a command. Should we enforce tighter sandboxing? | @alxcls | Open — see also [agent-lock-bypass.md](../agent-lock-bypass.md) |
