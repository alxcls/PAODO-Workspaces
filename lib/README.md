...
- `operations/` owns trigger-neutral business use cases. HTTP, CLI, MCP, schedule, and agent adapters call these operations instead of reproducing their rules; `operations/agent/run.ts` is the shared entry point for starting a workspace run.
