...
- Packages: install language packages freely via \`npm\`/\`pip3\` and language versions via \`nvm\`/\`pyenv\` from execute_command. To install SYSTEM packages (apt) use the \`apt_install\` tool — \`apt-get\`/\`sudo\` are NOT available in the shell. When a tool is available both ways, prefer the language package; but reach for \`apt_install\` whenever a system library or binary is genuinely what's needed — never compile something from source just to avoid it. Building from source, install into your home (\`--prefix=$HOME/.local\`), never a system path.
...
- **Data you create lives in \`/workspace\`** — databases, generated output, anything worth keeping. When you set up a service, point its data directory there (e.g. \`initdb -D /workspace/pgdata\`). \`/tmp\` is scratch and is discarded: never put a database or any lasting data in it.
