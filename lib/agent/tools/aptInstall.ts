...
// dockerExecAsRoot (docker exec -u 0 from the app server). To keep it from becoming an arbitrary
...
import { z } from "zod";
import { dockerExecAsRoot } from "@/lib/infra/containerManager";
...
export function buildAptInstallTool(workspaceId: string, workspaceDir: string) {
  return tool(
    async ({ packages }) => {
      ...
      const update = await dockerExecAsRoot(workspaceId, workspaceDir, ["apt-get", "update"]);
      if (update.code !== 0) {
        return `Error: apt-get update failed:\n${update.stderr || update.stdout}`;
      }

      const install = await dockerExecAsRoot(workspaceId, workspaceDir, [
