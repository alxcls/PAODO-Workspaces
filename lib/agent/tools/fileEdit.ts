  ...
  return tool(
    async ({ file_path, old_string, new_string, replace_all }) => {
      ...
      if (isHidden(workspaceId, relpath)) {
        return `Error: "${file_path}" is hidden [H] — its content is invisible to you and cannot be edited. Ask the user to reveal it (eye icon in the file tree) if it needs changing.`;
      }
