...
// GET classifies a file as text/image/binary (or serves raw bytes); PUT writes text; DELETE removes.
...
export interface FileBackend {
  ...
  // Called after a successful write or delete (workspace git snapshot). `message` describes the change.
...
export async function putFileContent(req: Request, be: FileBackend): Promise<Response> {
  ...
  const log = createLogger("api").child(be.logContext);
  try {
    const filePath = path.isAbsolute(body.path) ? body.path : path.join(be.dir, body.path);
    const resolved = await assertInsideWorkspace(be.dir, filePath);
