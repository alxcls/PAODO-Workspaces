...
import { archiveDrives } from "../lib/infra/drives/archive";
import { reportArchived, verifyAndReport } from "./archiveCli";

const USAGE = `Usage:
  npm run backup:drives -- <destination-dir-or-file>
  npm run backup:drives -- --verify <archive.tar.gz>`;
...
async function main(): Promise<void> {
  ...
  const result = await archiveDrives(destination);
